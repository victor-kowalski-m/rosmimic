...
class EnvGazebo(EB.EnvBase):
    ...
    def _fr3_camera_callback(self, msg):
        ...
        if msg.encoding == 'rgb8':
            self._fr3_camera_image = np.frombuffer(msg.data, dtype=np.uint8).reshape(msg.height, msg.width, 3)
        elif msg.encoding == 'bgr8':
            img = np.frombuffer(msg.data, dtype=np.uint8).reshape(msg.height, msg.width, 3)
            self._fr3_camera_image = img[:, :, ::-1]  # Convert BGR to RGB
        else:
            # For other encodings, keep as is
            self._fr3_camera_image = np.frombuffer(msg.data, dtype=np.uint8).reshape(msg.height, msg.width, -1)

    def _external_camera_callback(self, msg):
        """Callback for external camera images."""
        # Convert ROS Image message to numpy array directly
        if msg.encoding == 'rgb8':
            self._external_camera_image = np.frombuffer(msg.data, dtype=np.uint8).reshape(msg.height, msg.width, 3)
        elif msg.encoding == 'bgr8':
            img = np.frombuffer(msg.data, dtype=np.uint8).reshape(msg.height, msg.width, 3)
            self._external_camera_image = img[:, :, ::-1]  # Convert BGR to RGB
        else:
            # For other encodings, keep as is
            self._external_camera_image = np.frombuffer(msg.data, dtype=np.uint8).reshape(msg.height, msg.width, -1)
    ...
    def step(self, action):
        ...
        print("Gripper action:", action[7])
        # if action[7] < 0.02:  # Assuming a threshold for closing the gripper
        #     grasp_msg = GraspActionGoal()
        #     grasp_msg.goal.width = action[7]  # Desired gripper width
        #     grasp_msg.goal.force = 10 # Grasping force
        #     grasp_msg.goal.speed = 0.1  # Default speed
        #     grasp_msg.goal.epsilon.inner = 0.005
        #     grasp_msg.goal.epsilon.outer = 0.005
        ...
        # Wait for ROS callbacks to update
        rospy.sleep(0.1)
    ...
    def reset(self):
        ...
        # grasp_msg = GraspActionGoal()
        # grasp_msg.goal.width = 0.4  # Desired gripper width
        # grasp_msg.goal.force = 0 # Grasping force
        # grasp_msg.goal.speed = 0.1  # Default speed
        # grasp_msg.goal.epsilon.inner = 0.005
        # grasp_msg.goal.epsilon.outer = 0.005
        # self._gripper_grasp_pub.publish(grasp_msg)
        ...
        grasp_msg = MoveActionGoal()
        grasp_msg.goal.width = 0.6  # Desired gripper width
        grasp_msg.goal.speed = 0.2  # Default speed
        self._gripper_move_pub.publish(grasp_msg)
    ...
    def render(self, mode="human", height=None, width=None, camera_name=None, **kwargs):
        ...
        """
        if mode == "rgb_array":
            # return np.concatenate([self._current_obs['external_camera_image'], self._current_obs['fr3_camera_image']], axis=1)
